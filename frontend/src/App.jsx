    ...
    <div className="min-h-screen p-8 max-w-7xl mx-auto font-sans">
      ...
      <header className="flex justify-between items-center mb-12">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-red-600 rounded-2xl flex items-center justify-center shadow-[0_0_20px_rgba(220,38,38,0.4)]">
            <Flame className="text-white w-10 h-10" fill="currentColor" />
          </div>
          <div>
            <h1 className="text-6xl font-black tracking-tighter uppercase italic leading-none text-white selection:bg-red-500">
              UNIFIRE REGISTRY
            </h1>
            <p className="text-white/40 text-[10px] tracking-[0.3em] mt-3 font-black uppercase">
              DON BOSCO COLLEGE OF ENGINEERING, FATORDA, GOA
            </p>
          </div>
        </div>
        <div className="bg-red-600 rounded-xl px-8 py-4 text-center shadow-[0_0_30px_rgba(220,38,38,0.2)]">
      ...
      {(stats.overdue > 0 || stats.dueSoon > 0) && (
        <div className="bg-red-950/10 border border-red-500/10 rounded-xl p-3 mb-10 flex items-center gap-3 text-red-500/60 text-[10px] font-black uppercase tracking-widest">
          <AlertTriangle size={14} className="text-red-500/80" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        <div className="stat-card border-l-[3px] border-l-red-600">
          ...
          <div className="text-5xl font-black italic text-red-500 my-1">{stats.total}</div>
        ...
        <div className="stat-card border-l-[3px] border-l-green-500">
          ...
          <div className="text-5xl font-black italic text-green-500 my-1">{stats.upToDate}</div>
        ...
        <div className="stat-card border-l-[3px] border-l-yellow-500">
          ...
          <div className="text-5xl font-black italic text-yellow-500 my-1">{stats.dueSoon}</div>
        ...
        <div className="stat-card border-l-[3px] border-l-red-500/30">
          ...
          <div className="text-5xl font-black italic text-red-400 my-1">{stats.overdue}</div>
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        ...
        <div className="relative">
          <select 
            className="bg-[#121212] border border-white/5 rounded-lg px-6 py-3 text-[10px] font-black uppercase tracking-widest appearance-none min-w-[160px] outline-none cursor-pointer hover:border-white/10 transition-all"
            value={floorFilter}
        ...
        <button onClick={handleExport} className="flex items-center gap-2 px-6 py-3 rounded-lg border border-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-white/30 hover:bg-white/5 transition-all">
          <Download size={14} />
          Export CSV
        </button>
      </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#1a1a1a]">
                ...
                <th className="table-header">Last Refilled</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="table-cell">
                    <div className="text-white/80 font-black uppercase tracking-tight">{item.wing}</div>
                    <div className="text-[9px] text-white/20 font-black uppercase tracking-widest mt-1">{item.location_detail}</div>
                  </td>
                  <td className="table-cell font-mono font-black text-white/60">{item.last_refilled}</td>
                  <td className="table-cell font-mono font-black text-red-500/80">{item.due_date}</td>
                  <td className="table-cell">
                    <span className={`badge ${
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-[#151515]">
              <h3 className="text-xl font-black uppercase italic tracking-tight text-white flex items-center gap-3">
                ...
                {editingItem ? 'Edit Extinguisher' : 'Add New Unit'}
              </h3>
            ...
            <form onSubmit={handleSave} className="p-8 grid grid-cols-2 gap-6">
              <div className="col-span-2 space-y-2">
                ...
                <input 
                  required
                  className="w-full bg-black/40 border border-white/5 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-red-600 transition-all font-bold text-white"
              ...
              <div className="col-span-2 flex gap-4 mt-4">
                <button type="submit" className="flex-1 bg-red-600 text-white py-4 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-red-500 transition-all shadow-xl">
                  {editingItem ? 'Save Changes' : 'Register Unit'}
                </button>
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-white/5 text-white/40 py-4 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-white/10 transition-all">
                  Cancel
                </button>
              </div>
            </form>
